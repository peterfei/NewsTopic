...
class LFScrollTitleView: UIView {
    
    /// 存放标题模型的数组
    var titles = [LFHomeTopTitle]()
    /// 存放标题 label 数组
    var labels = [LFTitleLabel]()
    /// 存放 label 的宽度
    private var labelWidths = [CGFloat]()
    /// 顶部导航栏右边加号按钮点击
    var addBtnClickClosure: (() -> ())?
    /// 点击了一个 label
    var didSelectTitleLable: ((titleLabel: LFTitleLabel)->())?
    /// 向外界传递 titles 数组
    var titlesClosure: ((titleArray: [LFHomeTopTitle])->())?
    /// 记录当前选中的下标
    ...
    override init(frame: CGRect) {
        ...
        // 获取首页顶部标题数据
        LFNetworkTool.shareNetworkTool.loadHomeTitlesData { [weak self] (topTitles) in
            // 添加推荐标题
            let dict = ["category": "__all__", "name": "推荐"]
            let recommend = LFHomeTopTitle(dict: dict)
    ...
    func didSelectTitleLableClosure(closure:(titleLabel: LFTitleLabel)->()) {
        didSelectTitleLable = closure
    }
    
    /// 暴露给外界，向外界传递 topic 数组
    func titleArrayClosure(closure: (titleArray: [LFHomeTopTitle])->()) {
...
extension LFScrollTitleView {
    
    /// 添加 label
    private func setupTitlesLable() {
        for (index, topic) in titles.enumerate() {
            let label = LFTitleLabel()
            label.text = topic.name
            label.tag = index
            label.textColor = LFColor(235, g: 235, b: 235, a: 1.0)
    ...
    func titleLabelOnClick(tap: UITapGestureRecognizer) {
        guard let  currentLabel = tap.view as? LFTitleLabel else {
            return
        }
        oldIndex = currentIndex
        currentIndex = currentLabel.tag
        let oldLabel = labels[oldIndex]
        oldLabel.textColor = LFColor(235, g: 235, b: 235, a: 1.0)
