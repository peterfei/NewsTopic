...
class LFNewsTopic: NSObject {
    ...
    var video_detail_info: LFVideoDetailInfo?
    ...
    // 点击 『x』 按钮，弹出框内容
    var filter_words = [LFFilterWord]()
    
    var image_list = [LFImageList]()
    var middle_image: LFMiddleImage?
    var large_image_list = [LFLargeImageList]()
    ...
    init(dict: [String: AnyObject]) {
        ...
        if let filterWords = dict["filter_words"] as? [AnyObject] {
            for item in filterWords {
                let filterWord = LFFilterWord(dict: item as! [String: AnyObject])
        ...
        if let mediaDict = dict["media_info"] {
            media_info = LFMediaInfo(dict: mediaDict as! [String: AnyObject])
        }
        
        if let videoDetailInfo = dict["video_detail_info"] {
            video_detail_info = LFVideoDetailInfo(dict: videoDetailInfo as! [String : AnyObject])
        }
        
        if let middleImage = dict["middle_image"] {
            middle_image = LFMiddleImage(dict: middleImage as! [String: AnyObject])
        }
        ...
        if imageLists == nil || imageLists?.count == 0 {
                ...
                if video_detail_info?.video_id != nil || largeImageLists?.count > 0 {
                    ...
                    if largeImageLists?.count > 0 {
                        for index in largeImageLists! {
                            let largeImage = LFLargeImageList(dict: index as! [String : AnyObject])
            ...
            for item in imageLists! {
                let imageList = LFImageList(dict: item as! [String: AnyObject])
...
class LFLargeImageList: NSObject {
...
class LFVideoDetailInfo: NSObject {
    ...
    var detail_video_large_image: LFDetailVideoLargeImage?
    ...
    init(dict: [String: AnyObject]) {
        ...
        detail_video_large_image = LFDetailVideoLargeImage(dict: dict["detail_video_large_image"] as! [String: AnyObject])
    }
}

class LFDetailVideoLargeImage: NSObject {
